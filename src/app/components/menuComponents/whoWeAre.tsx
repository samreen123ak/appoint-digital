...
const WhoWeAreDropDown: React.FC<Props> = ({ isOpen, onClose }) => {
  const col1 = [
    { name: "About Us", href: "/whoWeAre/aboutUs" },
    { name: "Careers" },
  ];
    ...
    <div className="space-y-5">
      {items.map((item, i) => (
        <div key={i} className="flex items-center gap-5 font-bold text-sm">
          <FaDiamond className="text-primary text-3xl" />
          {item.href ? (
            <Link
              href={item.href}
              className="hover:text-primary transition font-bold text-4xl"
            >
              {item.name}
            </Link>
          ) : (
